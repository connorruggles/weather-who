...
var renderForecast = function (data) {
  // converts each forecast into a var arr
  renderForecastHeader();
  var arr = data.list;
  for (var i = 0; i < arr.length; i++) {
    // splits the array so you can only see each day at 12
...
var renderCurrentWeather = function (data) {
  ...
  for (var i = 0; i < data.list.length; i++) {
    ...
    // puts api weather info onto the page
...
var fetchCity = function () {
  ...
  var cityURL =
    "https://api.openweathermap.org/geo/1.0/direct?q=" + cityNameInput + "&limit=1&appid=b89c09787bf9106df63088418a47c76b";
  fetch(cityURL)
    .then(function (response) {
      return response.json();
    })
...
// call functions on each click and collect user input for local storage
...
// receives local storage and applies it to page
